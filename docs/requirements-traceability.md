...
| 4.9.3 | Speaking data privacy (retention, disclosure) | 🔨/🏗️ | Retention days + daily quota in module settings 🔨; S3 lifecycle 🏗️. Deletion reaches the search index too: the nightly reindex removes documents it did not write, so a transcript whose incremental removal was lost — or which was purged while the index was unreachable — stops being searchable rather than surviving indefinitely |
...
| 7.3 | API error envelope + correlationId | 🔨 | `lms/lms/language_platform/envelope.py` decorator used by module APIs. Covers **every** outcome, including unforeseen exceptions: those return 500 `INTERNAL_ERROR` with a generic message, the real cause going to the Error Log keyed by the correlation id rather than to the client. Frappe's control-flow exceptions (redirect, auth, session, CSRF) pass through untouched, and a failed call's partial writes are rolled back — returning a value puts the request on frappe's success path, which would otherwise commit them |
...
| 9.2 | KVKK technical rights (DSAR) | 🔨 | `LMS Data Request` (Export/Erasure, Pending→Approved→Completed, four-eyes gate: erasure cannot be self-approved) + `dsar.py` / `privacy_rules.py`. Export = full JSON of everything stored about the person, loaded per document so child tables (quiz results, placement answers, speaking rubric feedback) are included rather than only the rows wrapping them. Erasure = anonymise (academic records kept pseudonymously per "zorunlu saklama", personal free text deleted, speaking audio destroyed immediately). Pseudonymisation is real, not nominal: the User is renamed, which rewrites declared Link fields and — via frappe's `User.after_rename` — the `owner`/`modified_by` columns of every table, so the original address does not survive anywhere. This supersedes the earlier note that those columns were a residual needing separate migration; it was measured against this frappe version, not assumed. What remains for counsel is the *policy* split in `PERSONAL_DATA_SOURCES`: which sources are purged, scrubbed or retained under "zorunlu saklama" is a legal judgement, and financial records are deliberately retained against their own statutory clock. Self-service export for students |
